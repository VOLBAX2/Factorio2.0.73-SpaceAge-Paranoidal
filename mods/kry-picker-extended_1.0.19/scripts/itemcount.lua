...
--[Item Count]--
-------------------------------------------------------------------------------
-- original mod name: Item Count
-- original mod author: ThaPear
...
local function get_itemcount_counts(event)
    if not script.active_mods["yemtositemcount"] then
        ...
        if stack and (stack.prototype.stackable or stack.prototype.stack_size > 1) then
            local inventory_count = player.get_item_count({name=stack.name,quality=stack.quality})
            local vehicle_count
