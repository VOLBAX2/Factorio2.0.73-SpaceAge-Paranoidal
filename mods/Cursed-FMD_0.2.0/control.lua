...
script.on_event(defines.events.on_built_entity, function(event) des_building(event.entity, false) end)
script.on_event(defines.events.on_robot_built_entity, function(event) des_building(event.entity, false) end)
...
script.on_event(defines.events.on_player_selected_area, function(event)
    if event.item == "filter-drills" and #event.entities > 0 then
        ...
        storage.fmd.players[player.name] = event.entities
        open_gui(player)
    end
end)
...
script.on_event(defines.events.on_gui_click, function(event)
    ...
    local splitName = util.split(event.element.name,connector)
    if splitName[1] == "fmd" then
        local entities = storage.fmd.players[player.name]
...
function changeEntity(entity1, newSufix)
    local splitName = util.split(entity1.name,connector)
    local newEnt
	game.players[1].print(newSufix)
    if newSufix == "NO" then
        if not splitName[2] then
        else
            update_resources(entity1, splitName[2], "")
        ...
        if splitName[2] and splitName[2] == newSufix then
            ...
            if prototypes.get_entity_filtered({ {filter = "name", name = splitName[1]..connector..newSufix} })[splitName[1]..connector..newSufix] then
                if splitName[2] then
                ...
                local resProto = prototypes.get_entity_filtered({ {filter = "name", name = newSufix} })[newSufix]
    ...
    if remote.interfaces["Cursed-DB"] and oldSufix ~= newSufix and (oldSufix ~= "" or newSufix ~= "") then
        ...
        for i, drill in ipairs(drills) do
            if drill ~= entity then
                local splitName = util.split(drill.name,connector)
                if splitName[2] then
...
function open_gui(player)
    ...
    for n, ent in ipairs(entities) do
        local minerArr = util.split(ent.name, connector)
        local minerName = minerArr[1]
        local minerProto = prototypes.get_entity_filtered({ {filter = "name", name = minerName} })[minerName]
        if minerProto and minerProto.type == "mining-drill" then
    ...
    local tabs = {}
    for resName, res in pairs(storage.fmd.resources) do
        if possiblesCats[resName] then
            local proto = prototypes.get_entity_filtered({ {filter = "name", name = resName} })[resName]
    ...
    for guiName, arr in pairs(tabs) do
        if #arr > 0 then
            ...
            local frame = tabbed.add({ type="frame", style = "entity_frame", name = "fme_"..guiName })
            ...
            tableAll.add({ type="sprite-button", name="fmd"..connector.."NO_FILTER", number=(countAll-countCat), sprite="utility/missing_icon", style="slot_button", tooltip={"gui.reset-production-filters"} })
            for i, res in ipairs(arr) do
                tableAll.add({ type="sprite-button", name="fmd" .. connector .. res.name, number=(actualCat[res.name] or 0), sprite="entity/"..res.name, style="slot_button", tooltip=res.localised })
            end
        end
    end
    ...
    spend_frame.flowButtons.flowButtonsClose.add({ type = "button", name = "fmd" .. connector .. "btnCancelUpgrade", caption = {"gui.close"}, style = "red_back_button" })
...
function init_table()
    if not storage.fmd then
        storage.fmd = {}
    end
    if not storage.fmd.players then
        storage.fmd.players = {}
    end
    if not storage.fmd.tabs then
        storage.fmd.tabs = { }
        storage.fmd.tabs["no_tab"] = { caption = "[item=iron-ore]" }
    end
    local resources = {}
    for resourceName, res in pairs(prototypes.get_entity_filtered({ {filter = "type", type = "resource"} })) do
        if res.resource_category == resourceName then
            resources[resourceName] = true
        end
    end
	
    storage.fmd.resources = resources
end
...
--  match/prefix/sufix: search this in the resource name}
    init_table()
    if type(dats) == "table" and dats.name and dats.caption and (dats.match or dats.prefix or dats.sufix) and not storage.fmd.tabs[dats.name] then
        storage.fmd.tabs[dats.name] = dats
        return true
    end
    return false
end,
...
function(name)
    init_table()
    if name and storage.fmd.tabs[name] then
        storage.fmd.tabs[name] = nil
        return true
    end
    return false
end
