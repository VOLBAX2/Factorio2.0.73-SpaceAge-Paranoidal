...
if settings.startup[cfg1.mod_prefix .. "chemical-weapon-chemical-recipes"].value then
...
if settings.startup[cfg1.mod_prefix .. "vehicle-battery-enable"].value then
...
if TPlib.check_set_value(ft_ammo_type2, "consumption_modifier", 1) then --1.125
  ...
  if settings.startup[cfg1.mod_prefix .. "tank-flamethrower-fire-stream-incendiary"].value then
...
-- Poison colour
if settings.startup[cfg1.mod_prefix .. "ammo-colour-rearrange"].value then
  ...
  -- Make Poison capsule to use new yellow colour
  ...
  -- Swap Slowdown capsule to use blue colour
  TPlib.check_set_value(dr["capsule"]["slowdown-capsule"], "icon", "__base__/graphics/icons/poison-capsule.png")
  if TPlib.check_set_value(dr["projectile"]["slowdown-capsule"], {"animation", "filename"}, "__base__/graphics/entity/poison-capsule/poison-capsule.png") then
    dr["projectile"]["slowdown-capsule"].animation = 
    {
  ...
  TPlib.check_set_value(dr["sticker"]["slowdown-sticker"], {"animation", "tint"}, {r=0.000, g=0.663, b=1.000, a=0.694})  --{r = 1.000, g = 0.663, b = 0.000, a = 0.694}
  TPlib.check_set_value(dr["optimized-particle"]["slowdown-capsule-particle"], {"pictures", "sheet", "tint"}, {r=0.155, g=0.632, b=0.878, a=0.694})  --{ r = 0.878, g = 0.632, b = 0.155, a = 0.694 }
  TPlib.check_set_value(dr["optimized-particle"]["slowdown-capsule-particle-big"], {"pictures", "sheet", "tint"}, {r=0.155, g=0.632, b=0.878, a=0.694})  --{ r = 0.878, g = 0.632, b = 0.155, a = 0.694 }
end
...
TPlib.check_set_value(drad["discharge-defense-equipment"], "automatic", settings.startup[cfg1.mod_prefix .. "discharge-defense-equipment-automatic"].value)
if TPlib.check_set_value(PLD, {"attack_parameters", "ammo_type", "energy_consumption"}, settings.startup[cfg1.mod_prefix .. "personal-laser-defense-equipment-energy-consumption"].value) then  --"50kJ"
  local PLD_shot_energy = util.parse_energy(settings.startup[cfg1.mod_prefix .. "personal-laser-defense-equipment-energy-consumption"].value)
...
local cannon_force_condition = settings.startup[cfg1.mod_prefix .. "tank-cannon-force-condition"].value
