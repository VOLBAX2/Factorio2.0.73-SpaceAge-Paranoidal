...
local function cache_settings()
  ...
  config.minimum_distance_to_degradetiles    = settings.global["Noxys_Trees-minimum-distance-to-degradeable-tiles"]
      .value
...
local function process_chunk(surface, chunk)
  ...
  if config.kill_trees_near_unwanted then
    ...
    if config.deaths_by_pollution_bias > 0 then
      tokill = tokill +
          mathceil(surface.get_pollution { chunk.x * 32 + 16, chunk.y * 32 + 16 } / config.deaths_by_pollution_bias)
    end
    repeat
