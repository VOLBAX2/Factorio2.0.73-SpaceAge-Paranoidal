	...
	if script.on_tick ~= nil then 
		if type(script.on_tick)=="table" and type(script.on_tick.tick) == "function" then
			...
			if script.on_tick.priority ~= nil then
				if script.on_tick.priority == heroturrets.events.low_priority then
					table.insert(heroturrets.ticks_low,script.on_tick.tick)
				elseif script.on_tick.priority == heroturrets.events.medium_priority then
					table.insert(heroturrets.ticks_med,script.on_tick.tick)
				elseif script.on_tick.priority == heroturrets.events.high_priority then
...
local local_on_init = function()
	...
	if storage.heroturrets == nil then storage.heroturrets = {allow_pickup_rotations = false} end	
	for k=1, #heroturrets.on_init do local v = heroturrets.on_init[k]		
		v()
	end
	storage.heroturrets.initialized = true 
	end
...
script.on_configuration_changed(function(f) 
	...
	--[[if f.mod_changes["HeroTurretRedux"].old_version < "0.1.25" then	
...
local local_on_player_rotated_entity = function(event)
	local_on_standard_entity_event(event.entity,heroturrets.on_player_rotated_entity,heroturrets.on_player_rotated_entity_by_name)
	end
...
-- FIX LATER script.on_event({on_player_joined_game,defines.events.on_player_created},local_on_player_spawned)
