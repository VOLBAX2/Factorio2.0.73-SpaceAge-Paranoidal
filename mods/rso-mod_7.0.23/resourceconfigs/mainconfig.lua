...
require("resourceconfigs.trickyoldnick")
require("resourceconfigs.cargoships")
require("resourceconfigs.pyalternativeenergy")
require("resourceconfigs.spacewithoutspace")
require("resourceconfigs.muluna")
require("resourceconfigs.picksbase")
...
--	local pyModsVersion = versionValue(script.active_mods["pycoalprocessing"])
--	local use194Configs = pyModsVersion >= version194
	...
	config.aquilo = {}
	config.vulcanus = {}
	config.vulcanus.resourceChanceMultiplier = 1.5
	config.aquilo.resourceChanceMultiplier = 0.5
	config.gleba.resourceChanceMultiplier = 0.25
...
--	
--	if script.active_mods["fpp"] then
--		fillFppConfig(config)
--	end

	-- if script.active_mods["bobenemies"] and not prototypes.entity["alien-ore"] or useEnemiesInPeaceMod then  -- if the user has peacemod installed he probably doesn't want that RSO spawns them either. remote.interfaces["peacemod"]
		-- fillBobEnemies(config)
	-- end

--	if script.active_mods["Natural_Evolution_Enemies"] then
--		fillNEConfig(config)
--	end
--
--	-- Roadworks mod
...
--
--	-- Andrew's mods (ores)
--	if script.active_mods["andrew-ore"] then
...
--
	...
	if prototypes.entity["y-res1"] then
...
--		if script.active_mods["BukketMod"] then
...
--	end
--
--	if script.active_mods["SenpaisOverhall"] then
--		fillSenpaisConfig(config)
--	end
--
--	if script.active_mods["Beyond"] then
--		fillBeyondConfig(config)
--	end
--
--	if script.active_mods["infinium-ore"] then
--		fillInfiniumConfig(config)
--	end
--
--	if script.active_mods["AnonyMods"] then
--		fillAnonyModsConfig(config)
--	end
--
--	if script.active_mods["cncs_Sulfur_Mod"] then
--		fillSulfurConfig(config)
--	end
--
--	if script.active_mods["PrimordialOoze"] then
...
--	end
--
--	if script.active_mods["SigmaOne_Nuclear"] then
--		fillSigmaOneNuclearConfig(config)
--	end
--
--	if script.active_mods["xander-mod"] or script.active_mods["xander-mod-th"] then
--		fillXanderConfig(config)
--	end
--
--	if script.active_mods["xander-mod-v1"] then
--		fillXander1Config(config)
--	end
--
--	if script.active_mods["Darkstar_utilities"] or script.active_mods["Darkstar_utilities_Low_Spec"]
--	or script.active_mods["Darkstar_utilities_fixed"] then
...
--
--	if script.active_mods["HydraulicPumpjacks"] then
--		fillHydraulicPumpjacksConfig(config)
--	end
--
	if script.active_mods["NapusMod"] then
...
--
--	if script.active_mods["liquid-science"] then
...
--	end
--
--	if script.active_mods["Dp77s-Sulfur-Mod"] then
--		fillDp77SulfurConfig(config)
--	end
--
--	if script.active_mods["AllMinable"] then
--		fillAllMinableConfig(config)
--	end
--
--	if script.active_mods["FMRx"] then
--		fillFmrxConfig(config)
--	end
--
--	if script.active_mods["Dp77s-FactorioPlus-Ores"] then
--		fillDp77OresConfig(config)
--	end
--
--	if script.active_mods["Bio_Industries"] then
--		fillBioIndustriesConfig(config)
--	end
--
--	if script.active_mods["KPOT_Titanium"] then
--		fillKpotConfig(config)
--	end
--
--	if script.active_mods["FiniteWater"] then
--		fillFiniteWaterConfig(config)
--	end
--
--	if script.active_mods["Mutabor"] then
--		fillMutaborConfig(config)
--	end
--
--	if script.active_mods["Fixed_Tiberium"] or script.active_mods["Fixed_Tiberium_okta"] then
--		fillTiberiumConfig(config)
--	end
--
--	if script.active_mods["tiny-overhaul"] or script.active_mods["extended-industries"] then
...
--
--	if script.active_mods["leighzermorphite"] then
--		fillLeighzerMorphiteConfig(config)
--	end
--
--	if script.active_mods["leighzerscienceores"] then
--		fillLeighzerScienceOres(config)
--	end
--
--	if script.active_mods["leighzersciencebottling"] then
--		fillLeighzerScienceBottling(config)
--	end
--
--	if script.active_mods["leighzercheetahore"] then
--		fillLeighzerCheetahOreConfig(config)
--	end
--
--	if script.active_mods["aix_matter"] then
--		fillAiXMatterConfig(config)
--	end
--
--	if script.active_mods["SimpleSilicon"] then
--		fillSimpleSiliconConfig(config)
--	end
--
--	if script.active_mods["ReforestedWood"] then
...
--	if script.active_mods["zzz_modders_science_pack"] then
--		fillZzzModdersSciencePackConfig(config)
--	end
--
--	if script.active_mods["bottled_science"] then
...
--
--	if script.active_mods["IndustrialRevolution"] or script.active_mods["IndustrialRevolution3"] then
--		fillIndustrialRevolutionConfig(config)
--	end
--
--	if script.active_mods["Krastorio"] then
...
--	end
--	
--	if script.active_mods["geothermalgen"] then
...
--	if script.active_mods["dualores"] then
...
--	end
	...
	if script.active_mods["bzchlorine"] then
...
--	end
--
--	if script.active_mods["RitnGlass"] then
...
--	if script.active_mods["yaiom"] then
...
--	fillEnhancedRecipesConfig(config)
	...
	if script.active_mods["cargo-ships"] then
		fillCargoShipsConfig(config)
	end
	
	if script.active_mods["SpaceAgeWithoutSpace"] then
		fillSpaceAgeWithoutSpaceConfig(config)
	end

	if script.active_mods["planet-muluna"] then
		fillMulunaConfig(config)
	end

	if script.active_mods["PicksBaseMod"] then
		fillPicksBaseConfig(config)
	end

	-- BobOres
	if script.active_mods["bobores"] then
		fillBoboresConfig(config)
	-- elseif script.active_mods["5dim_ores"] then
		-- fill5dimConfig(config)
	end

	if script.active_mods["qatmore"] then
	...
	if script.active_mods["angelsrefining"] then
		...
		if not script.active_mods["Atlas_NormalOresForAngels"] then
