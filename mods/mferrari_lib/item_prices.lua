 ...
 * ITEM PRICING ROUTINE  by MFERRARI
 ...
 v 2.1  (08/12/2024) - maraxis planet
 v 2.2  (07/01/2025) - cerys moon , updated must_have_prices
 v 2.3  (10/03/2025) - fix infinite loop when recipe with no ingredient - Azgard
...
local Excluded_recipes_part_names = {"_expensive"}

local Excluded_recipes = {"angels-plate-iron", "angels-plate-tin", "player-port", "loader", "fast-loader",
                          "express-loader", "rocket-part", "belt-immunity-equipment", "small-plane", "tc_import_chest",
                          "tc_export_chest", "artillery-targeting-remote", "tc_portable_loader_coal_in",
                          "tc_portable_loader_in", "tc_portable_loader_out"}

local must_have_prices = {
    ["automation-science-pack"] = 39,
    ["logistic-science-pack"] = 115,
    ["military-science-pack"] = 182,
    ["chemical-science-pack"] = 486, -- 935
    ["production-science-pack"] = 1384, -- 4151,
    ["utility-science-pack"] = 1635, -- = 4922,
    ["electromagnetic-science-pack"] = 2000,
    ["metallurgic-science-pack"] = 2000
    -- ["space-science-pack"] = 1500,
    -- ["agricultural-science-pack"] = 2000,
...
local Basic_Item_Prices = {
    ...
    ["promethium-asteroid-chunk"] = 10000,
...
["uvaz-plate"]= 50,]]
    ...
    ["maraxsis-sand"] = 1,
    ["maraxsis-salt"] = 2,
    ["maraxsis-limestone"] = 2,
    ["maraxsis-saline-water"] = 0.1,
    ["maraxsis-coral"] = 2,
    ["maraxsis-atmosphere"] = 0.01,
    ["maraxsis-tropical-fish"] = 4,
    ["maraxsis-salted-fish"] = 4,
    ["maraxsis-hydrogen"] = 0.5,
    ["maraxsis-oxygen"] = 0.5,

    ["palusium-ore"] = 1,
    ["palusium-powder"] = 1000,
    ["palusium-plate"] = 50,

    -- cerys
    ["nuclear-waste-solution"] = 30,
    ["plutonium-238"] = 1200,
    ["plutonium-239"] = 60000,
    ["cerys-nuclear-scrap"] = 3,
    ["methane-ice"] = 5,
    ["methane"] = 15,
    ["cerys-nitrogen-rich-minerals"] = 5
        ...
        if not in_list(Items_List, o.name) then
            if (not only_item_plus_recipe) or (prototypes.recipe[o.name]) then
                if not in_list(Excluded_recipes, o.name) then
                    ...
                    if Basic_Item_Prices[o.name] then
                        storage.Priced_Items[o.name] = Basic_Item_Prices[o.name]
                    end
                end
            end
        end
...
function Pricing_Add_Item_Price(name)
    ...
    local notPriced = {}
    if storage.Priced_Items[name] == nil and Basic_Item_Prices[name] == nil then
        table.insert(notPriced, name)
    end

    if debug_prices then
        Log('   ===inicio===>  ' .. name)
    end
    ...
    while #notPriced > 0 do
        ...
        if debug_prices then
            Log(' *not priced > ' .. RecipeName)
        end

        if prototypes.recipe[RecipeName] then
            if debug_prices then
                Log(' >found recipe for: ' .. RecipeName)
            end
            ...
            -- excluded part names
            for k = 1, #Excluded_recipes_part_names do
                if string.find(recipe.name, Excluded_recipes_part_names[k]) then
                    exit_now = true
                    break
                end
            end

            -- no ingredients on receipe
            if tti == 0 then
                exit_now = true
            end
            ...
            if not exit_now then
                for k, i in pairs(ingred) do
                    ...
                    if price == nil then
                        if debug_prices then
                            Log(' !no price for ingredient: ' .. nome)
                        end
                        if not in_list(notPriced, nome) then
                            table.insert(notPriced, nome) -- adiciona no fim da fila
                        else
                            exit_now = true
                            if debug_prices or debug_lost_items then
                                Log(' infinite loop detected - exiting because of ' .. name .. ' --dupe cause->' .. nome)
                            end
                        end
                        found = false
                        break
                        ...
                        if debug_prices then
                            Log(' $ found price for: ' .. nome .. ' $' .. price)
                        end
                    end
            ...
            if found then
                ...
                if Tprice > 0 then
                    local results = 1
                    if recipe.products and recipe.products[1] and recipe.products[1].amount and
                        recipe.products[1].amount > 1 then
                        results = recipe.products[1].amount
                    end

                    storage.Priced_Items[RecipeName] = math.ceil(Tprice / results)
                end
                if debug_prices then
                    Log(' $$$$$>>> total for: ' .. RecipeName .. ' $$ ' .. Tprice)
                end
            ...
            if debug_prices then
                Log(' !!!!!!!! NOT POSSIBLE - EXITING recipe ' .. RecipeName)
            end -- could be cost zero 
            break
        end -- tem recipe
...
function Pricing_Create_Prices_List(only_item_plus_recipe)
    ...
    for k, v in Sort_a_Table(storage.Priced_Items, function(t, a, b)
        return t[b] > t[a]
    end) do
        table.insert(Sorted_Item_Prices, {
            name = k,
            price = v
        })
        if log_all_price_table then
            Log(k .. ' = ' .. v)
        end
    end
...
function get_itens_to_pay_for_price(force, price, quant, except)
    ...
    while tries > 0 do
        ...
        for k, i in pairs(storage.Sorted_Item_Prices) do
            ...
            if force.recipes[i.name] and force.recipes[i.name].enabled and (not force.recipes[i.name].hidden) and
                i.price >= vmin and (not in_list(never, i.name)) and (not in_list(ship_tiles, i.name)) and
                (not string.find(i.name, "barrel", nil, true)) then
                table.insert(opt, i)
            end
        end
        if #opt < 5 then
            ...
            for k, i in pairs(storage.Sorted_Item_Prices) do
                if force.recipes[i.name] and force.recipes[i.name].enabled and (not force.recipes[i.name].hidden) and
                    (not in_list(ship_tiles, i.name)) and (not in_list(never, i.name)) then
                    table.insert(opt, i)
                end
            end
        end
        ...
        if except then
            ...
            table.insert(except, "parameter")
            for x = #opt, 1, -1 do
                for y = 1, #except do
                    if string.find(opt[x].name, except[y], nil, true) then
                        table.remove(opt, x)
                        break
                    end
                end
        ...
        while #items < quant and #opt > 1 and cost < price do
            ...
            if q > stack * 6 then
                q = stack * 6
            end -- using a limit of 6 stacks max
        ...
        tries = tries - 1
        if cost < price * 1.2 and cost > price * 0.7 then
            break
        end
    end
...
function get_market_itens_for_force(force, forb_items, minimum_value, allow_disabled)
    if not minimum_value then
        minimum_value = 1
    end
    local opt = {}
    forb_items = forb_items or {}

    for k, i in pairs(storage.Sorted_Item_Prices) do
        if (force.recipes[i.name] and force.recipes[i.name].enabled and (not force.recipes[i.name].hidden)) or
            (allow_disabled and (not force.recipes[i.name].enabled)) then
            ...
            if i.price <= 65500 and i.price >= minimum_value and allow then
                if not in_list(forb_items, i.name) then
                    table.insert(opt, {
                        name = i.name,
                        price = i.price
                    })
...
function IP_check_reset_prices(only_item_plus_recipe)
    if not storage.Sorted_Item_Prices then
