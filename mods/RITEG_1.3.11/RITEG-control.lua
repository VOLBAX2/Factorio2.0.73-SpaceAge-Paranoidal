...
function try_init()
  ...
  if not storage.rtgs then storage.rtgs = {} end
  if not storage.tick_id then storage.tick_id = 1 end
  if not storage.total_rtgs then storage.total_rtgs = 0 end
end

function update_settings()
  storage.threshold = settings.global[mod_name .. "autodeconstruct-health-threshold"].value
end

function update_ritegs()
  for i, rtg in pairs(storage.rtgs) do
    local entity = rtg.entity
    if entity and entity.valid then -- added in 1.3.1
      local max_health = entity.max_health
      rtg.health_threshold = max_health * storage.threshold / 100
    else
      -- do nothing
    end
  end
...
function add_all_rtgs_to_table()
  ...
  for _, surface in pairs(game.surfaces) do
    for _, rtg_name in pairs(rtg_names_list) do
      if prototypes.entity[rtg_name] then
        ...
        for _, entity in pairs(found_entities) do
          ...
          add_glow(entity)
          table.insert(storage.rtgs, rtg)

          storage.total_rtgs = storage.total_rtgs + 1
        end
      end
    end
  end
  -- log('storage.total_rtgs = ' .. storage.total_rtgs)
end

function make_rtg_prototype(data)
  local entity = data.entity
  local max_health = entity.max_health
  local quality_multiplier = 1.0 + (entity.quality.level * 0.3)
  local rtg = {
    name = data.name,
    first_tick = data.tick,
    first_energy = RTG_prop[data.name].energy * quality_multiplier, -- 40 GJ
    entity = entity,
    first_power = (entity.health * 10 ^ 3),                         -- 600 kW
    health_threshold = max_health * storage.threshold / 100,
    pause_to_tick = data.tick + 120
  }
  rtg.half_life = rtg.first_energy * ln_2 / rtg.first_power
  rtg.power = power_of_RITEG(rtg)

  log('starting power: ' .. rtg.power * 60 .. ' half life: ' .. rtg.half_life .. ' starting energy: ' .. rtg
    .first_energy)
  return rtg
end
...
function onBuildHandler(event)
  ...
  storage.total_rtgs = storage.total_rtgs + 1
  -- game.print ('Total RITEGs: '..storage.total_rtgs)
end

function remove_rtg(tick_id)
  table.remove(storage.rtgs, tick_id)
  storage.total_rtgs = storage.total_rtgs - 1
  -- game.print('['..tick_id..'/'..storage.total_rtgs..'] '..'RITEG was deleted')
end
...
script.on_event(defines.events.on_tick, function(event)
  if storage.total_rtgs == 0 then return end
  local tick_id = storage.tick_id

  if tick_id > storage.total_rtgs then
    tick_id = 1
  end
  local rtg = storage.rtgs[tick_id]
  ...
  local power_was = rtg.power_was or
      (entity.health * (50 / 3)) -- 1000/60=50/3 is 10000 Joule/tick (it's 600 000 Watt or 600 kW) divide thrue 600 Watt
